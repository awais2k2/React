...
export default function Customersize() {
  ...
  const submithandler = (e) => {
    ...
    if (
      ...
      adduserdatahandler(state);
      async function adduserdatahandler(state) {
        const response = await fetch(
          "https://react-http-ea7a3-default-rtdb.firebaseio.com/customers.json",
          {
            method: "POST",
  ...
  async function customerdatahandler() {
    const response = await fetch(
      "https://react-http-ea7a3-default-rtdb.firebaseio.com/customers.json"
    );
  ...
  return (
    <>
      <div className="w-[90%] mx-auto p-[30px] text-[20px] border-[1px] border-solid mt-[60px] pb-0 pl-0 pr-0">
        <form onSubmit={submithandler} className="flex flex-col gap-[40px]">
          <div className="flex justify-between gap-[30px] items-center px-[20px]">
      ...
      <SearchCustomer cus={customers}></SearchCustomer>
    </>
  );
}
